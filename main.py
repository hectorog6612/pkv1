...
bot = Client("bot",
             bot_token= "6914434441:AAFLB8It4SjkF-qntGT2Kr1r51lEcmXYbn8",
             api_id= 27803618,
...
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            if "jw-prod" in url:
                cmd = f'yt-dlp -U "{name}.mp4" "{url}"'
            else:
                cmd = f'yt-dlp -f "{ytf}" "{url}" -o "{name}.mp4"'

            try:  
                
                cc = f'**Vid_id  »** {str(count).zfill(3)}\n**Title  »** {name1} {res} ᴊᴏʜɴᴡɪᴄk.mkv\n**Batch »** {raw_text0}\n\n**Download by »** {raw_text3}\n\n'
                cc1 = f'**Vid_id  »** {str(count).zfill(3)}\n**Title »** {name1} ᴊᴏʜɴᴡɪᴄk.pdf \n**Batch »** {raw_text0}\n\n**Download by »** {raw_text3}\n\n'
                if "drive" in url:
                    try:
